...
<div class="page-head_agile_info_w3l">
	...
	<div class="privacy">
		<div class="container">
                        ...
                        @foreach($checkout as $key => $value)
							<tr class="rem{{($key % 2) + 1}}">
								...
								<td class="invert-image">
										<img style="width: 120px;" src="{{ Storage::disk(config('voyager.storage.disk'))->url($value->product->main_image) }}" alt=" ">
								</td>
								<td class="invert">
									<div class="quantity">
										<div class="quantity-select">
                                            ...
                                            <select onchange="changeQuantity({{$value->id}}, this.value, {{$value->product->price_per_kg}}, {{$value->quantity}})" name="quantity{{$value->id}}" id="quantity{{$value->id}}">
                                            @for($i=1;$i<=100;$i++)
                                            {{$value->product}}
											<option value="{{($value->product->minimum_quantity * $i)}}" {{($value->quantity == $value->product->minimum_quantity * $i) ? "selected" : ""}}>{{($value->product->minimum_quantity * $i) < 1000 ? ($value->product->minimum_quantity * $i) . " g" : ($value->product->minimum_quantity * $i)/1000 . " kg"}}</option>
											@endfor
                                            </select>
										</div>
									</div>
								...
								<td class="invert" id="price{{$value->id}}">₹{{$value->product->price_per_kg * $value->quantity / 1000}}</td>
								<td class="invert">
                                    <a href="{{url('remove_from_cart', $value->id)}}">
									<div class="rem">
                            ...
                            <tr class="rem2">
								...
								<td class="invert" colspan="2" id="subtotalText">{{$total}}</td>
										
                            </tr>
                            <tr class="rem1">
								<td class="invert" colspan="4">Delivery Charge</td>
								<td class="invert" colspan="2" id="deliveryChargeText">Enter Pincode First</td>			
                            </tr>
                            <tr class="rem1">
								<td class="invert" colspan="4">Final Total</td>
								<td class="invert" colspan="2" id="finalPriceText">Enter Pincode First</td>
            ...
            @if(count($checkout) > 0)
            <div class="controls">
				<input class="billing-address-name" type="number" name="pincode" id="pincode" placeholder="Pincode" required="">
            </div>
            <button onclick="checkPincode()" id="verifyPincodeButton">Verify</button>
            <button onclick="editPincode()" id="editPincodeButton" style="display: none;">Edit Pincode</button>
    ...
    <script>
        ...
        $.ajax({
            ...
            success: function (response) {
                document.getElementById('subtotalText').innerText = '₹' + (Number(document.getElementById('subtotal').value) - oldprice + newprice);
                document.getElementById('subtotal').value = Number(document.getElementById('subtotal').value) - oldprice + newprice;
                if(Number(document.getElementById('deliveryCharge').value) > 0)
                {
                    document.getElementById('finalPriceText').innerText = '₹' + ((Number(document.getElementById('subtotal').value)) + (Number(document.getElementById('deliveryCharge').value)));
                    document.getElementById('finalPrice').value = (Number(document.getElementById('subtotal').value)) + (Number(document.getElementById('deliveryCharge').value));
                }
        ...
        if (pin == "" || pin == null) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    {
                        document.getElementById('deliveryChargeText').innerText = '₹' + response.data.delivery_charge;
